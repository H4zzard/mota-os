...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (!agentsLoading && agentList.length > 0 && selectedAgent) {
      const updated = agentList.find(
        a => a.shortName === selectedAgent.shortName,
      );
  ...
  function selectSession(id: string) {
    ...
    const matched = agentName
      ? (agentList.find(a => a.shortName === agentName) ?? null)
      : null;
  ...
  function handleAgentChange(a: AgentWithConfig) {
    const enriched = agentList.find(
      ag => ag.id === a.id || ag.shortName === a.shortName,
    );
  ...
  const handleSend = useCallback(
    async (
      ...
      const history = messages.map(m => ({
        ...
        content: m.content
          .filter(
            (b): b is { kind: "text"; content: string } => b.kind === "text",
          )
          .map(b => b.content)
          .join("\n"),
      }));
                ...
                setMessages(prev =>
                  prev.map(m =>
                    m.id === aiMsgId
                      ? {
                          ...m,
                          content: [{ kind: "text" as const, content: snap }],
                        }
                      : m,
                  ),
                );
