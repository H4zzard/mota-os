...
export function ChatWindow({
  ...
  const [deviceAuth, setDeviceAuth] = useState<DeviceStartResponse | null>(
    null,
  );
  ...
  const geminiReconnectHref =
    "/api/auth/google/logout?returnTo=/api/auth/google/login%3FreturnTo%3D%2Fchat";
        ...
        try {
          ...
          if (data.status === "denied") {
            if (!cancelled) {
              setDeviceAuth(null);
              setDeviceError(
                "A autorização foi negada. Inicie novamente para gerar outro código.",
              );
            }
            return;
          }
          ...
          if (!cancelled) {
            ...
            setDeviceError(
              data.error_description ??
                "Não foi possível concluir o login GPT.",
            );
          }
          return;
        } catch {
          if (!cancelled) {
            setDeviceAuth(null);
            setDeviceError(
              "Falha de rede ao validar o login. Tente novamente.",
            );
          }
          return;
        }
    ...
    try {
      ...
      if (
        ...
        setDeviceError(
          data.error ??
            data.details ??
            "Não foi possível iniciar o login GPT por dispositivo.",
        );
        setDeviceAuth(null);
        return;
      }
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <a
            ...
            style={{
              ...
              borderColor: geminiAuthReady
                ? "rgba(29,78,216,0.24)"
                : "var(--border-color)",
            }}
            title={
              geminiAuthReady
                ? "Trocar ou renovar a conta Google do Gemini"
                : "Conectar conta Google para usar Gemini"
            }
          >
          ...
          <button
            ...
            title={
              rightPanelOpen ? "Fechar painel" : "Abrir painel de contexto"
            }
          >
            {rightPanelOpen ? (
          ...
          <div className="px-6 py-6 space-y-6 max-w-4xl mx-auto">
            {messages.map((msg, i) => (
              <ChatMessage
                ...
                index={i}
                onSendToRocket={
                  msg.role === "assistant" ? handleSendToRocket : undefined
                }
                onRegenerate={
        ...
        <div className="grid grid-cols-2 gap-2 w-full">
          {quickPrompts.map(p => (
            ...
            >
              <MessageSquare
                size={13}
                className="text-mota-500 shrink-0 mt-0.5"
              />
              <span
...
function CompanySelector() {
  const { currentCompany, allowedCompanies, loading, setCurrentCompany } =
    useCompany();
