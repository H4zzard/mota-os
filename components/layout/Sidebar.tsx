...
export function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  useEffect(() => {
    if (isCollapsed) {
      savedGroupsRef.current = new Set(openGroupsRef.current);
      setOpenGroups(new Set());
      const t = setTimeout(() => setContentCollapsed(true), 150);
      return () => clearTimeout(t);
    } else {
      setContentCollapsed(false);
      setOpenGroups(savedGroupsRef.current);
    }
  }, [isCollapsed]);
  ...
  useEffect(() => {
    function fetchUnread() {
      fetch("/api/announcements/unread-count")
        .then(r => r.json())
        .then(data =>
          setUnreadNews(typeof data.count === "number" ? data.count : 0),
        )
        .catch(() => {});
    }
    fetchUnread();
  ...
  return (
    ...
    >
      {/* Header — logo sempre visível, texto e botão fazem fade */}
      <div
        className="flex items-center justify-between px-3 h-14 shrink-0 border-b"
      ...
      {/* Botão expandir (só no estado contraído) */}
      <AnimatePresence initial={false}>
        {contentCollapsed && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.15 }}
            className="px-2.5 pb-3 shrink-0"
          >
            <button
              onClick={onToggle}
